...
class AtendimentoService extends MetaModelService
{
    ...
    public function distribuiSenha($unidade, $usuario, $servico, $prioridade, $nomeCliente, $documentoCliente)
    {
        ...
        //verificar qtd max de senha no dia
        $dataAtual = date("Y-m-d",time());
        $dataServico = date_format($su->getDataContador(),"Y-m-d");
        $qtdSenhas = $su->getMaxsenhas();
        $contadorSenhas = $su->getContador() + 1;

        if($dataAtual == $dataServico ){
            if($contadorSenhas > $qtdSenhas){
                throw new Exception(_('Excedido o limite máximo de senhas para esse serviço'));
            }else{
                $horaAtual = time();
                $horaServico = strtotime($su->getHoralimite());
                if($horaAtual <= $horaServico){
                    $su->setContador($contadorSenhas);
                    $this->em->merge($su);
                    $this->em->flush();
                }else{
                    throw new Exception(_('Fora do horário limite para liberação de senhas'));
                }
            }
        }else{
            $su->setContador(0);
            $su->setDataContador(new DateTime());
            $this->em->merge($su);
            $this->em->flush();
        }
